...
miniRT features a full hierarchical UI system built on top of the **mlxui**
library (a custom minilibx UI toolkit). The UI provides:

- A **scene hierarchy list** (left panel) showing all objects, lights, and
  the camera
- **Edit panels** for modifying object geometry, materials, and render
  settings in real time
- **On-display information**: FPS counter, info text, and render mode
  switch buttons with logos
- **Selection system** via click, drag (rubber-band), and scene list
- **Color theme** with customizable Material Design-like palette
- **TTF font rendering** via the custom `font_renderer` library

The UI is built from a **hierarchical tree** (`t_htree`) of **branches**
(`t_hbranch`), where each branch is a UI component (box, button, textbox,
slider, colorpicker, etc.). The tree is populated once during initialization
and rebuilt dynamically when the selection or edit mode changes.
...
The left panel's edit section uses a **block-based** layout system defined in
`include/ui/edit_pannel/blocks.h`.
...
Rebuilt via `rebuild_obj_edit()` when the selection changes. Contains:

- **Object title** (name + type)
- **Geometry block**: Position (vec3), rotation (vec3), and type-specific
  fields (radius for spheres, normal for planes, etc.)
- **Material block**: All PBR properties with sliders and colorpickers,
  plus texture map selectors per channel (kd, normal, roughness, ambient,
  opacity, metalness)
...
- **Camera parameters**: Position, rotation, FOV, focus distance, lens
  radius, exposure — all with sliders
- **Export block**: Two buttons — "Export Scene" (F11, writes `.rt` file)
  and "Export Image" (F12, schedules render task → PPM)

### Dynamic Rebuild

When the user clicks a different edit mode tab, `rebuild_inner_edit()` is
called, which:

1. Destroys the current inner edit panel's components
2. Calls `populate_obj_edit()` or `populate_render_edit()` depending on
   `edit_mode`
3. Calls `precompute_hbranch()` on the entire edit panel subtree

---

## Scene List

The scene list (`include/ui/scene_list.h`) displays all objects in the
scene hierarchy. Each entry is a `t_scene_entry`:
...
Entries are added by type:

- `add_cam_entry()` — Camera
- `add_light_entry()` — Each light
- `add_obj_entry()` — Each object (sphere, plane, mesh)

Max entries: `MAX_SCENE_ENTRIES = 50`. Clicking an entry in the scene list
triggers `select_entry()`, which calls `select_obj()` on the corresponding
`t_object`.
...
1. On mouse button down (`MLCLICK`), `select_zone()` records the start
   position and begins tracking mouse movement.
2. On mouse move, `select_mouse_pos()` updates `mouse_pos[END]`.
3. `draw_select()` renders a selection rectangle overlay on the screen.
4. On mouse button up, objects inside the drag rectangle are selected:
   - `rearrange_order()` ensures START < END in both axes
   - `select_zone()` detects which objects are within the rectangle via
     `is_click_inside()` (tests against the on-display UI bounding box,
     or the full screen)

### Click Selection

Clicking an object in the scene list calls `select_obj(data, obj)`, which:
1. Pushes `obj` onto `data->ui.selection.selected`
2. Calls `update_scene_select()` to highlight the entry in the scene list
3. Calls `rebuild_obj_edit()` to show the object's edit panel
4. Sets `edit_mode` to `OBJ_EDIT_MODE`

### Rendered Selection Highlight

`rasterize_selected()` in `loop.c` draws outlines around all selected
objects using the CPU rasterizer: spheres → circles, triangles → triangle
outlines, planes → quads, lights → diamond shapes.
...
A `TEXTBOX` at the top-left of the screen (`anchor=LT`, `font_size=4`,
color `YELLOW`). Updated every frame via `update_fps()`.

### Info Display

A `TEXTBOX` positioned at `(50, 240)` with `font_size=2` and opacity 15
(mostly transparent). Updated via `update_info_display()` which writes
information about:
- Selected object name and type
- Global render parameters

### Render Mode Switch

A `BUTTON_GROUP` with `GROUP_SWITCH` behavior (radio-button style, 4
buttons) positioned at top-right (`anchor=RT`). Each button contains an
`IMAGE` with a `.pam` logo:
...
The switched index is aliased to `data->params.render_mode`, so clicking a
logo immediately changes the render mode.
...
The UI uses the custom **font_renderer** library (`lib/font_renderer/`) to
render TTF fonts. The font is loaded at UI initialization:
...
The same font is used system-wide for all text boxes. Font size is specified
per-component (values 2, 3, 4 map to scaled sizes in the font renderer).

The font file lives at `assets/fonts/JetBrainsMono-ExtraLight.ttf` and is
also symlinked at `asset/fonts/JetBrainsMono-ExtraLight.ttf`.
