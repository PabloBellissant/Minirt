...
void	phong_render(t_data *data, t_img_data *img)
{
	...
	intersect_loop(data->buffers.rays, data->buffers.hits,
		&data->scene, data->buffers.hits_count);
	data->buffers.hits_count = compact_hits_inplace(data->buffers.hits,
			data->buffers.hits_count);
	sample_colors_loop(data, data->scene.texture.data, data->scene.mat.data,
		data->buffers.hits_count);
	if (data->params.normal_debug)
		normal_debug_loop(&data->buffers, data->buffers.hits_count);
	else
		shade_loop(&data->buffers, &data->scene, data,
			data->buffers.hits_count);
