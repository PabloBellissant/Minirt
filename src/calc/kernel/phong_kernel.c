...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/02/04 21:29:57 by pabellis          #+#    #+#             */
/*   Updated: 2026/02/04 21:29:57 by pabellis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	phong_kernel(t_opencl *c, t_data *data, t_img_data *img)
{
	...
	scene = &data->scene;
	if (set_args(c, &c->kernel.phong, scene) != 0)
		return (-1);
	err = clEnqueueNDRangeKernel(c->queue, c->kernel.phong, 2, NULL,
...
static int	set_args(t_opencl *c, cl_kernel *kernel, t_scene *scene)
{
	...
	err |= clSetKernelArg(*kernel, 12, sizeof(cl_float3), &scene->ambient);
	if (err != CL_SUCCESS)
		return (error(pack_err(RT_ID, RT_E_KERNEL), FL, LN, FC));
	return (0);
}
