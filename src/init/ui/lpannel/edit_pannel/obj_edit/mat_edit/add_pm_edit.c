...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/02/18 17:15:20 by jaubry--          #+#    #+#             */
/*   Updated: 2026/02/19 20:37:48 by jaubry--         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	add_pm_edit(t_data *data, t_hbranch *obj_edit, int *y_size, t_mat *mat)
{
	...
	ret = 0;
	tex = get_vector_value(&data->scene.texture, mat->metalness_id.index);
	if (!ret && is_tex_editable(tex))
	{
		bl = (t_block_util){.data = data, .comp = obj_edit, .y_size = y_size,
			.tex_ptr = tex, .name = "Metalness"};
